...
GitHub Actions (`.github/workflows/deploy.yml`) → GHCR → SSH деплой на VPS sel1. Триггер — push в `main`. CI прогоняет `prisma migrate deploy` + `next build` против тестового postgres, потом собирает образ и деплоит.
...
- **Код панели:** отдельный проект `~/Obsidian/seo-dashboard` (Node/Express + Chart.js). Полная операционка — `seo-dashboard/OPERATIONS.md` (где на sel1, как редеплоить, SSL/DNS). Прод: контейнер `seo-dashboard` на sel1 (сеть webproxy:8090).
